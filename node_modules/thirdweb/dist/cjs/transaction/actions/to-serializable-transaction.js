...
async function toSerializableTransaction(options) {
    ...
    const isZkSync = await (0, isZkSyncChain_js_1.isZkSyncChain)(options.transaction.chain);
    if (isZkSync) {
        const { getZkGasFees } = await Promise.resolve().then(() => require("./zksync/send-eip712-transaction.js"));
        const { gas, maxFeePerGas, maxPriorityFeePerGas } = await getZkGasFees({
            transaction: options.transaction,
            from: options.from ? (0, address_js_1.getAddress)(options.from) : undefined,
        });
