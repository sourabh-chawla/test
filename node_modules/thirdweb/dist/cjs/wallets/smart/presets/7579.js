...
function erc7579(options) {
    const saltHex = options.overrides?.accountSalt &&
        ox_1.Hex.validate(options.overrides.accountSalt)
        ? options.overrides.accountSalt
        : ox_1.Hex.fromString(options.overrides?.accountSalt ?? "");
        ...
        overrides: {
            ...
            execute(accountContract, transaction) {
                return (0, execute_js_1.execute)({
                    ...
                    async asyncParams() {
                        return {
                            mode: ox_1.Hex.padRight("0x00", 32), // single execution
                            executionCalldata: ox_1.AbiParameters.encodePacked(["address", "uint256", "bytes"], [
            ...
            executeBatch(accountContract, transactions) {
                return (0, execute_js_1.execute)({
                    ...
                    async asyncParams() {
                        return {
                            mode: ox_1.Hex.padRight("0x01", 32), // batch execution
                            executionCalldata: ox_1.AbiParameters.encode([
                                {
                                    type: "tuple[]",
                                    components: [
            ...
            async getAccountNonce(accountContract) {
                ...
                const withValidator = ox_1.Hex.from(`${defaultValidator}${ox_1.Hex.fromNumber(entryPointNonce).slice(42)}`);
                return ox_1.Hex.toBigInt(withValidator);
            },
